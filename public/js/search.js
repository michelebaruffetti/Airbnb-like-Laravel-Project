...
$(document).ready(function () {
  ...
  var servizi_selezionati = []; // al click dei servizi 

  $('.servizi').on('click', function () {
    // recupero l'id del servizio selezionato
    var servizio_selezionato = $(this).val(); // se non è incluso nell'array servizi selezionati lo pusho altrimenti lo tolgo
  ...
  function chiamata(latitude, longitude, raggio, stanze, bagni) {
    $.ajax({
      ...
      success: function success(data) {
        ...
        var template = Handlebars.compile(source); // ciclo i dati della risposta

        for (var i = 0; i < data.response.length; i++) {
          // creo un array per i servizi di ogni appartamento
          var servizi_appartamento = []; // se la descrizione è lunga la tronco
          ...
          var sentinella = 0; // ciclo i servizi di ogni appartamento
          ...
          var html = template(context); // se i servizi selezionati sono 0 disegno la card 

          if (servizi_selezionati.length == 0) {
            $('#contenitore-appartamenti').append(html); // altrimenti ciclo i servizi selezionati dall'utente
          } else {
            for (var y = 0; y < servizi_selezionati.length; y++) {
              // transformo in numeri i valori
              var servizi_selezionati_utente = Number(servizi_selezionati[y]); // se i servizi selezionati sono inclusi nell'array dei servizi dell'appartamento
